...
export default function Index({navigation, route}) {
  ...
  const onAccept = () => {
    dispatch(
      bookingActions.onAccept(route.params?.item, ({success, data}) => {
        if (success) {
          setDetail(data);
        }
      }),
    ...
    if (detail?.allowCancel || detail?.allowAccept) {
      return (
        <View style={Styles.buttonContent}>
          {detail?.allowCancel && (
            <Button onPress={onCancel} style={Styles.flex}>
              {t('cancel')}
            </Button>
          )}
          {detail?.allowCancel && detail?.allowAccept && (
            <SizedBox width={16} />
          )}
          {detail?.allowAccept && (
            <Button onPress={onAccept} style={Styles.flex}>
              {t('accept')}
            </Button>
          )}
        </View>
